...
public final class LlvmIfNode extends LlvmStatementNode {

    /**
     * The condition of the {@code if}. This in a {@link LlvmExpressionNode} because we require a
     ...
     */
    @Child private LlvmExpressionNode conditionNode;

    /** Statement (or {@link LlvmBlockNode block}) executed when the condition is true. */
    @Child private LlvmStatementNode thenPartNode;

    /** Statement (or {@link LlvmBlockNode block}) executed when the condition is false. */
    @Child private LlvmStatementNode elsePartNode;
    ...
    public LlvmIfNode(SourceSection src, LlvmExpressionNode conditionNode, LlvmStatementNode thenPartNode, LlvmStatementNode elsePartNode) {
        super(src);
    ...
    public void executeVoid(VirtualFrame frame) {
        ...
        if (condition.profile(evaluateCondition(frame))) {
            ...
            /* Execute the else-branch (which is optional according to the Llvm syntax). */
    ...
    private boolean evaluateCondition(VirtualFrame frame) {
        try {
            ...
            /*
             * The condition evaluated to a non-boolean result. This is a type error in the Llvm
             * program. We report it with the same exception that Truffle DLlvm generated nodes use
             * to report type errors.
             */
