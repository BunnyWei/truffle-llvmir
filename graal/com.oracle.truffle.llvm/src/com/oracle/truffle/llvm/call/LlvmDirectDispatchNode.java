 ...
 */
final class LlvmDirectDispatchNode extends LlvmAbstractDispatchNode {

    /** The cached function. */
    private final LlvmFunction cachedFunction;
    ...
    /**
     * The next entry of the polymorphic inline cache, either another {@link LlvmDirectDispatchNode}
     * or a {@link LlvmUninitializedDispatchNode}.
     */
    @Child private LlvmAbstractDispatchNode nextNode;

    protected LlvmDirectDispatchNode(LlvmAbstractDispatchNode next, LlvmFunction cachedFunction) {
     ...
     * <p>
     * Since Llvm is a quite simple language, the benefit of the inline cache is quite small: after
     * checking that the actual function to be executed is the same as the
     * {@link LlvmDirectDispatchNode#cachedFunction}, we can safely execute the cached call target.
     ...
     * lookup of the call target is usually much more complicated than in Llvm. In addition, caching
    ...
    protected Object executeDispatch(VirtualFrame frame, LlvmFunction function, Object[] arguments) {
        ...
        if (this.cachedFunction == function) {
            ...
            try {
                 ...
                 * maintained by the LlvmFunction object is change. To avoid a check for that, we
                 * use an Assumption that is invalidated by the LlvmFunction when the change is
                 * performed. Since checking an assumption is a no-op in compiled code, the line
                 * below does not add any overhead during optimized execution.
                 */
