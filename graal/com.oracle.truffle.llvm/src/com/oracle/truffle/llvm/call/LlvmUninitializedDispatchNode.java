 ...
 */
final class LlvmUninitializedDispatchNode extends LlvmAbstractDispatchNode {

    /**
     * When we reach this method, all the previous cache entries did not match the function. If the
     * cache is still small enough, we extend it by adding another {@link LlvmDirectDispatchNode}.
     * If the cache reached its maximum size, we replace the whole dispatch chain with a
     * {@link LlvmGenericDispatchNode}.
     */
    @Override
    protected Object executeDispatch(VirtualFrame frame, LlvmFunction function, Object[] arguments) {
        ...
        /*
         * Count the number of LlvmDirectDispatchNodes we already have in the cache. We walk the
         * chain of parent nodes until we hit the LlvmCallNode. We know that a LlvmCallNode is
         * always present.
         */
        Node cur = this;
        int depth = 0;
        while (cur.getParent() instanceof LlvmAbstractDispatchNode) {
            cur = cur.getParent();
            depth++;
        }
        LlvmInvokeNode invokeNode = (LlvmInvokeNode) cur.getParent();

        LlvmAbstractDispatchNode replacement;
        if (function.getCallTarget() == null) {
            /* Corner case: the function is not defined, so report an error to the user. */
            // throw new LlvmException("Call of undefined function: " + function.getName());
            System.out.println("Call of undefined function: " + function.getName());
            return null;
        } else if (depth < INLINE_CACHE_SIZE) {
            /* Extend the inline cache. Allocate the new cache entry, and the new end of the cache. */
            LlvmAbstractDispatchNode next = new LlvmUninitializedDispatchNode();
            replacement = new LlvmDirectDispatchNode(next, function);
            /* Replace ourself with the new cache entry. */
            replace(replacement);

        } else {
            /* Cache size exceeded, fall back to a single generic dispatch node. */
            replacement = new LlvmGenericDispatchNode();
